...
def extract_pitch_crepe(audio : AudioSegment ,model_capacity='medium'):
    """
    ...
    """
    time, frequency, confidence, activation = crepe.predict(np.array(audio.get_array_of_samples()), 16000, step_size=int(audio.duration_seconds*1000.0), model_capacity=model_capacity)
    # 이상치에 덜 민감한 평균(mean)을 사용하여 계산하는 코드
    trimmed_mean = frequency
    print(frequency)
    lower_bound = librosa.midi_to_hz(48)
    upper_bound = librosa.midi_to_hz(84)
    ...
    if np.any(valid_indices):
        ...
        print(trimmed_frequency,trimmed_mean)
    else:
